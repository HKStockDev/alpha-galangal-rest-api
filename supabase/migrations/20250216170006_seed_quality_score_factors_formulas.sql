...
insert into public.formulas (key, name, output_type, definition) values
  ...
  ('hedge_fund_conviction', 'Hedge Fund Conviction', 'number', '{"type":"weighted_zscore","terms":[{"w":0.35,"f":"pct_in_top_10"},{"w":0.25,"f":"avg_time_in_top_10"},{"w":0.25,"f":"avg_time_held"},{"w":-0.15,"f":"turnover"}]}'::jsonb),
  ...
  ('hedge_fund_quality_score', 'Hedge Fund Quality Score', 'number', '{"type":"composite","weights":{"hedge_fund_performance":0.3,"hedge_fund_risk":0.25,"hedge_fund_conviction":0.2,"hedge_fund_institutional_strength":0.15,"hedge_fund_positioning":0.1}}'::jsonb)
on conflict (key) do nothing;
