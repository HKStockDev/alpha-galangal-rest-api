...
const LYNCH_FACTOR_KEY = 'lynch_score';
const LYNCH_PROMPT_KEY = 'lynch_score';
const LYNCH_SOURCE = 'lynch_llm';
const LYNCH_MODEL_VERSION = 'v1';
const LYNCH_PERIOD_KEY = 'snapshot';
...
type LynchLlmResult = {
  ...
  dimensions: {
    ...
    industry_tailwinds: number;
    scalability: number;
    valuation_vs_growth: number;
  };
  reasons_for: string[];
...
@Injectable()
export class LynchScoreService {
  private readonly logger = new Logger(LynchScoreService.name);
  ...
  private async getActivePromptVersion(): Promise<{
    ...
    for (const key of expandFormulaKeyAliases(LYNCH_PROMPT_KEY)) {
  ...
  private parseAndNormalizeLlmPayload(rawText: string): LynchLlmResult {
    ...
    const dim = {
      revenue_growth: clamp(coerceNumber(dimensions.revenue_growth) ?? 0, 0, 100),
      industry_tailwinds: clamp(coerceNumber(dimensions.industry_tailwinds) ?? 0, 0, 100),
      scalability: clamp(coerceNumber(dimensions.scalability) ?? 0, 0, 100),
      valuation_vs_growth: clamp(coerceNumber(dimensions.valuation_vs_growth) ?? 0, 0, 100),
    };
    ...
    return {
      model: typeof parsed.model === 'string' ? parsed.model : 'lynch',
      score,
  ...
  async calculateScores(options: {
    ...
    const empty: LynchScoreCalculateResult = {
      tickersRequested: 0,
      tickersWithData: 0,
      scoresWritten: 0,
      errors: [],
    ...
    if (!prompt) {
      empty.errors.push({
        ticker: '_',
        message: 'Active Lynch prompt not found. Ensure prompt key lynch_score is seeded.',
      });
      return empty;
    }

    let factorRow: { id: string } | null = null;
    for (const key of expandFormulaKeyAliases(LYNCH_FACTOR_KEY)) {
    ...
    if (!factorRow?.id) {
      empty.errors.push({
        ticker: '_',
        message: `Factor ${LYNCH_FACTOR_KEY} not found`,
      });
      return empty;
    }
    ...
    for (const t of targets) {
      try {
        ...
        upsertRows.push({
          ...
          source: LYNCH_SOURCE,
          ingested_at: now,
          model_version: LYNCH_MODEL_VERSION,
          period_key: LYNCH_PERIOD_KEY,
        });
        scoreRows.push({
          ticker: t.ticker,
          score: out.score,
  ...
  async loadCurrentScores(options: {
    ...
    let q = this.adminClient
      .from('v_security_lynch_scores')
      .select('ticker, lynch_score, lynch_label, lynch_confidence, lynch_summary, lynch_updated_at')
      .order('lynch_score', { ascending: false });
    if (options.tickers?.length) {
      q = q.in('ticker', options.tickers.map((t) => t.trim().toUpperCase()).filter(Boolean));
    }
    if (options.minScore != null && Number.isFinite(options.minScore)) q = q.gte('lynch_score', options.minScore);
    if (options.maxScore != null && Number.isFinite(options.maxScore)) q = q.lte('lynch_score', options.maxScore);
    if (options.limit != null && options.limit > 0) q = q.limit(options.limit);
    const { data, error } = await q;
    if (error) {
      empty.errors.push({ ticker: '_', message: `Failed to load Lynch scores: ${error.message}` });
      return empty;
    }
    const rows = ((data ?? []) as Record<string, unknown>[])
      .filter((r) => typeof r.ticker === 'string' && coerceNumber(r.lynch_score) != null)
      .map((r) => {
        const s = coerceNumber(r.lynch_score) ?? 0;
        const label = typeof r.lynch_label === 'string' ? r.lynch_label : normalizeLabel(s);
        return {
          ...
          confidence: clamp(coerceNumber(r.lynch_confidence) ?? 0, 0, 1),
          summary: typeof r.lynch_summary === 'string' ? r.lynch_summary : '',
          updatedAt: r.lynch_updated_at != null ? String(r.lynch_updated_at) : null,
        };
      });
