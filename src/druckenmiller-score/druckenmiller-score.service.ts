...
const DRUCKENMILLER_FACTOR_KEY = 'druckenmiller_score';
const DRUCKENMILLER_PROMPT_KEY = 'druckenmiller_score';
const DRUCKENMILLER_SOURCE = 'druckenmiller_llm';
const DRUCKENMILLER_MODEL_VERSION = 'v1';
const DRUCKENMILLER_PERIOD_KEY = 'snapshot';
...
type DruckenmillerLlmResult = {
  model?: string;
  score: number;
  label: 'positive' | 'neutral' | 'negative';
  confidence: number;
  dimensions: {
    industry_cycle_alignment: number;
    earnings_revision_direction: number;
    sector_momentum: number;
    macro_positioning: number;
  };
  reasons_for: string[];
...
@Injectable()
export class DruckenmillerScoreService {
  private readonly logger = new Logger(DruckenmillerScoreService.name);
  ...
  private async getActivePromptVersion(): Promise<{
    ...
    for (const key of expandFormulaKeyAliases(DRUCKENMILLER_PROMPT_KEY)) {
  ...
  private parseAndNormalizeLlmPayload(rawText: string): DruckenmillerLlmResult {
    ...
    const dim = {
      industry_cycle_alignment: clamp(coerceNumber(dimensions.industry_cycle_alignment) ?? 0, 0, 100),
      earnings_revision_direction: clamp(coerceNumber(dimensions.earnings_revision_direction) ?? 0, 0, 100),
      sector_momentum: clamp(coerceNumber(dimensions.sector_momentum) ?? 0, 0, 100),
      macro_positioning: clamp(coerceNumber(dimensions.macro_positioning) ?? 0, 0, 100),
    };
    ...
    return {
      model: typeof parsed.model === 'string' ? parsed.model : 'druckenmiller',
      score,
  ...
  ): Promise<DruckenmillerLlmResult> {
  ...
  async calculateScores(options: {
    ...
    const empty: DruckenmillerScoreCalculateResult = {
      tickersRequested: 0,
      tickersWithData: 0,
      scoresWritten: 0,
      errors: [],
    ...
    if (!prompt) {
      empty.errors.push({
        ticker: '_',
        message: 'Active Druckenmiller prompt not found. Ensure prompt key druckenmiller_score is seeded.',
      });
      return empty;
    }

    let factorRow: { id: string } | null = null;
    for (const key of expandFormulaKeyAliases(DRUCKENMILLER_FACTOR_KEY)) {
    ...
    if (!factorRow?.id) {
      empty.errors.push({
        ticker: '_',
        message: `Factor ${DRUCKENMILLER_FACTOR_KEY} not found`,
      });
      return empty;
    }
    ...
    const scoreRows: DruckenmillerScoreRow[] = [];
    ...
    for (const t of targets) {
      try {
        ...
        upsertRows.push({
          ...
          updated_at: now,
          source: DRUCKENMILLER_SOURCE,
          ingested_at: now,
          model_version: DRUCKENMILLER_MODEL_VERSION,
          period_key: DRUCKENMILLER_PERIOD_KEY,
        });
        scoreRows.push({
          ticker: t.ticker,
          score: out.score,
  ...
  async loadCurrentScores(options: {
    ...
    let q = this.adminClient
      .from('v_security_druckenmiller_scores')
      .select('ticker, druckenmiller_score, druckenmiller_label, druckenmiller_confidence, druckenmiller_summary, druckenmiller_updated_at')
      .order('druckenmiller_score', { ascending: false });
    if (options.tickers?.length) {
      q = q.in('ticker', options.tickers.map((t) => t.trim().toUpperCase()).filter(Boolean));
    }
    if (options.minScore != null && Number.isFinite(options.minScore)) q = q.gte('druckenmiller_score', options.minScore);
    if (options.maxScore != null && Number.isFinite(options.maxScore)) q = q.lte('druckenmiller_score', options.maxScore);
    if (options.limit != null && options.limit > 0) q = q.limit(options.limit);
    const { data, error } = await q;
    if (error) {
      empty.errors.push({ ticker: '_', message: `Failed to load Druckenmiller scores: ${error.message}` });
      return empty;
    }
    const rows = ((data ?? []) as Record<string, unknown>[])
      .filter((r) => typeof r.ticker === 'string' && coerceNumber(r.druckenmiller_score) != null)
      .map((r) => {
        const s = coerceNumber(r.druckenmiller_score) ?? 0;
        const label = typeof r.druckenmiller_label === 'string' ? r.druckenmiller_label : normalizeLabel(s);
        return {
          ...
          confidence: clamp(coerceNumber(r.druckenmiller_confidence) ?? 0, 0, 1),
          summary: typeof r.druckenmiller_summary === 'string' ? r.druckenmiller_summary : '',
          updatedAt: r.druckenmiller_updated_at != null ? String(r.druckenmiller_updated_at) : null,
        };
      });
