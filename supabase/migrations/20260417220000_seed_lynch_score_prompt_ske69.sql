-- SKE-69: Seed Peter Lynch score prompt for alpha_galangal_committee_lynch_score
  ...
  v_schema jsonb := jsonb_build_object(
    ...
    'properties', jsonb_build_object(
      ...
      'dimensions', jsonb_build_object(
        ...
        'properties', jsonb_build_object(
          'revenue_growth', jsonb_build_object('type', 'number'),
          'industry_tailwinds', jsonb_build_object('type', 'number'),
          'scalability', jsonb_build_object('type', 'number'),
          'valuation_vs_growth', jsonb_build_object('type', 'number')
        )
      ),
...
You are evaluating whether {{ticker}} fits the investment style of Peter Lynch.
Score the company from 0 to 100 based on how strongly it matches Lynch-style growth-at-a-reasonable-price investing.
Focus on:
...
"dimensions": {
"revenue_growth": 0,
"industry_tailwinds": 0,
"scalability": 0,
"valuation_vs_growth": 0
},
"reasons_for": [],
"reasons_against": [],
"summary": ""
...
BEGIN
  ...
  IF v_org_id IS NULL THEN
    RAISE NOTICE 'seed_lynch_score_prompt_ske69: no organizations; skip';
    RETURN;
  END IF;
  ...
  VALUES (
    ...
    'Alpha Galangal Committee: Lynch Score',
    'number',
    jsonb_build_object('type', 'llm', 'model', 'lynch'),
    'LLM Peter Lynch score (0-100)',
    'Peter Lynch-style subscore used by Alpha Galangal Committee.',
    'organization',
    'MASTER_MODEL',
    'llm',
    1,
  ...
  IF v_formula_id IS NULL THEN
    ...
    WHERE key = 'alpha_galangal_committee_lynch_score'
    LIMIT 1;
  END IF;
  ...
  VALUES (
    ...
    'formula',
    'Peter Lynch Score',
    'LLM prompt for Peter Lynch-style GARP score (0-100) with label, confidence, dimensions, and reasons.'
  )
  ON CONFLICT (key) DO UPDATE
    SET name = EXCLUDED.name,
        description = EXCLUDED.description,
  ...
  SELECT
    ...
    1,
    'active',
    'You are a growth-at-a-reasonable-price equity analyst focused on Peter Lynch-style growth investing.',
    v_user_prompt,
    v_schema,
    'SKE-69 seed prompt',
    'gemini-2.0-flash',
