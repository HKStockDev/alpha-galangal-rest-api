...
const SUBJECT = 'Your Precision password was changed';
...
export function renderPasswordChangedEmail(ctx: PasswordChangedEmailContext): {
  ...
  const bodyHtml = `
    <p style="margin:0 0 12px 0;">Your Precision account password was changed on <strong>${escapeHtml(changedAt)} UTC</strong>.</p>
    <p style="margin:0;">If you did not make this change, contact support immediately and reset your password.</p>
  `;

  const text = `Your Precision account password was changed on ${changedAt} UTC.
...
— The Precision team`;
  ...
  return {
    ...
    html: renderTransactionalEmailLayout({
      preheader: 'Your Precision password was changed.',
