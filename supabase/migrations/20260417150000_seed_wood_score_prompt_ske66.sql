-- SKE-66: Seed Cathie Wood score prompt for alpha_galangal_committee_wood_score
  ...
  v_schema jsonb := jsonb_build_object(
    ...
    'properties', jsonb_build_object(
      ...
      'dimensions', jsonb_build_object(
        ...
        'properties', jsonb_build_object(
          'innovation_exposure', jsonb_build_object('type', 'number'),
          'technology_leverage', jsonb_build_object('type', 'number'),
          'future_market_expansion', jsonb_build_object('type', 'number'),
          'disruption_potential', jsonb_build_object('type', 'number')
        )
      ),
...
You are evaluating whether {{ticker}} fits the investment style of Cathie Wood.
Score the company from 0 to 100 based on alignment with innovation-driven growth investing.
Focus on:
technology leverage
innovation exposure
...
limited innovation exposure
low scalability
mature saturated industries
...
"confidence": 0.0,
"dimensions": {
"innovation_exposure": 0,
"technology_leverage": 0,
"future_market_expansion": 0,
...
BEGIN
  ...
  IF v_org_id IS NULL THEN
    RAISE NOTICE 'seed_wood_score_prompt_ske66: no organizations; skip';
    RETURN;
  END IF;
  ...
  VALUES (
    ...
    'Alpha Galangal Committee: Wood Score',
    'number',
    jsonb_build_object('type', 'llm', 'model', 'cathie_wood'),
    'LLM Cathie Wood score (0-100)',
    'Cathie Wood-style subscore used by Alpha Galangal Committee.',
    'organization',
    'MASTER_MODEL',
    'llm',
    1,
  ...
  IF v_formula_id IS NULL THEN
    ...
    WHERE key = 'alpha_galangal_committee_wood_score'
    LIMIT 1;
  END IF;
  ...
  VALUES (
    ...
    'formula',
    'Cathie Wood Score',
    'LLM prompt for Cathie Wood-style score (0-100) with label, confidence, dimensions, and reasons.'
  )
  ON CONFLICT (key) DO UPDATE
    SET name = EXCLUDED.name,
        description = EXCLUDED.description,
  ...
  SELECT
    ...
    1,
    'active',
    'You are a growth-focused equity analyst specialized in Cathie Wood-style innovation-driven, disruptive technology investing.',
    v_user_prompt,
    v_schema,
    'SKE-66 seed prompt',
    'gemini-2.0-flash',
