...
update public.formulas set display_formula = '0.35×z(pct_top10) + 0.25×z(avg_time_top10) + 0.25×z(avg_held) − 0.15×z(turnover)', description = 'Concentration and holding period (top 10, avg time held) vs turnover.' where key = 'hedge_fund_conviction';
...
update public.formulas set display_formula = '0.30×performance + 0.25×risk + 0.20×conviction + 0.15×institutional + 0.10×positioning', description = 'Composite quality score combining performance, risk, conviction, institutional strength, and positioning (each sub-score normalized 0–100).' where key = 'hedge_fund_quality_score';
